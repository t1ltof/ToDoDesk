import { CalendarDays, Download, FolderKanban, Inbox, ListTodo, Upload } from 'lucide-react'
...
export default function Sidebar(): JSX.Element {
  const { data, activeView, setActiveView, load } = useAppStore()
  ...
  const handleImport = async (mode: 'replace' | 'new-project'): Promise<void> => {
    const result = await window.tododesk.importData(mode)
  ...
  return (
    <aside className="flex h-full w-64 flex-col border-r border-surface-border bg-surface-elevated">
      ...
      <div className="space-y-1 border-t border-surface-border p-3">
        ...
        <button
          type="button"
          onClick={() => void handleImport('new-project')}
          className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-sm text-gray-300 hover:bg-surface-border/60"
        >
          <Upload size={16} />
          Импорт (новый проект)
